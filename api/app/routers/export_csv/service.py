...
class ExportService:
    ...
    async def export_bulk(self) -> Dict[str, Any]:
        ...
        bundle = {
            "metadata": {
                ...
                "exported_by": self.ctx.user.login,
        ...
        # Usuwamy puste klucze, do których user nie miał dostępu
    ...
    def _validate_access(self, entity_type: str):
        ...
        if entity_type in restricted:
            if not (entity_type == "user" and self.ctx.user_type == "group_admin"):
                raise exceptions.AccessDeniedError(f"You lack permissions to: {entity_type}")

    def _process_data(self, entity_type: str, raw_data: List[Any]) -> List[Dict[str, Any]]:
        ...
        for item in raw_data:
            if not self.ctx.is_admin and hasattr(item, "team_id"):
    ...
    def _map_machine(self, item: models.Machines):
        ...
        return {
            ...
            "metadata": {
                "agent_prometheus": item.machine_metadata.agent_prometheus if item.machine_metadata else False,
                "ansible_access": item.machine_metadata.ansible_access if item.machine_metadata else False,
            },
    ...
    def _map_maps(self, item: models.Map):
        ...
        return {
            ...
            "segments": [{"n1": s.node1_name, "n2": s.node2_name} for s in item.segments],
            "equipment": [{"n": e.name, "type": e.eq_type, "x": e.x, "y": e.y} for e in item.equipment]
        }
