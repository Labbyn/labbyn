...
class HistoryService:
    ...
    def _apply_user_visibility(self, stmt) -> sql.Select:
        """
        Apply security filters to restrict log visibility based on WHO performed the action.

        - Global Admins see all logs.
        - Group Admins see logs generated by users who share at least one team with them.
        - Regular users see logs generated by users from their teams, but USER entity logs are hidden.
        """
        if self.ctx.is_admin:
            return stmt

        user_teams = self.ctx.team_ids
        if not user_teams:
            return stmt.where(sql.false())

        allowed_users_stmt = (
            sql.select(models.UsersTeams.user_id)
            .where(models.UsersTeams.team_id.in_(user_teams))
            .scalar_subquery()
        )

        stmt = stmt.where(models.History.user_id.in_(allowed_users_stmt))

        if not self.ctx.is_group_admin:
            return stmt.where(models.History.entity_type != models.EntityType.USER)
