...
def test_security_scanner_all_endpoints_have_auth():
    ...
    for route in app.routes:
        if isinstance(route, (APIRoute, APIWebSocketRoute)):
            ...
            if any(path in route.path for path in ["/auth", "/docs", "/openapi.json", "/static", "/users"]):
                continue

            signature = get_typed_signature(route.endpoint)

            has_ctx = any(
                param.annotation is RequestContext or "RequestContext" in str(param.annotation)
    ...
    assert not missing_auth, f"Detected endpoints without RequestContext: {missing_auth}"
...
def test_security_mass_assignment_user_promotion(test_client, rbac_data_suite):
    ...
    res = ac.patch(f"/db/users/{rbac_data_suite['user_a_id']}",
                   json=payload,
...
def test_security_input_injection_robustness(test_client, rbac_data_suite):
    ...
    for pattern in payloads:
        res = ac.get(f"/db/users/?name={pattern}", headers=rbac_data_suite["user_a_header"])
...
def test_security_history_bola_leak(test_client, service_header_sync, rbac_data_suite):
    ...
    ac.patch(f"/db/users/{rbac_data_suite['admin_b_id']}",
...
def test_security_unauthorized_rollback_attempt(test_client, rbac_data_suite, service_header_sync):
    """
...
def test_security_token_behavior_after_password_change(test_client, rbac_data_suite, service_header_sync):
    """
    ...
    login_res = ac.post("/auth/login", data={
